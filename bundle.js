var bundle = (function (stream,path,util,events) {
'use strict';
...
function through (write, end, opts) {
  ...
  stream$$2.readable = stream$$2.writable = true;
  stream$$2.paused = false;

//  stream.autoPause   = !(opts && opts.autoPause   === false)
  stream$$2.autoDestroy = !(opts && opts.autoDestroy === false);

  stream$$2.write = function (data) {
    write.call(this, data);
    return !stream$$2.paused
  };

  function drain() {
    while(buffer.length && !stream$$2.paused) {
      var data = buffer.shift();
      if(null === data)
        return stream$$2.emit('end')
      else
        stream$$2.emit('data', data);
    }
  }

  stream$$2.queue = stream$$2.push = function (data) {
//    console.error(ended)
    if(_ended) return stream$$2
  ...
  stream$$2.on('end', function () {
    ...
    if(!stream$$2.writable && stream$$2.autoDestroy)
  ...
  function _end () {
    ...
    if(!stream$$2.readable && stream$$2.autoDestroy)
      stream$$2.destroy();
  }

  stream$$2.end = function (data) {
    if(ended) return
    ended = true;
    if(arguments.length) stream$$2.write(data);
    _end(); // will emit or queue
    return stream$$2
  };

  stream$$2.destroy = function () {
    if(destroyed) return
    destroyed = true;
    ended = true;
    buffer.length = 0;
    stream$$2.writable = stream$$2.readable = false;
    stream$$2.emit('close');
    return stream$$2
  };

  stream$$2.pause = function () {
    if(stream$$2.paused) return
    stream$$2.paused = true;
    return stream$$2
  };

  stream$$2.resume = function () {
    if(stream$$2.paused) {
      stream$$2.paused = false;
...
var json = typeof JSON === 'object' ? JSON : jsonify;
...
function Results (stream$$2) {
    this.count = 0;
    this.fail = 0;
    this.pass = 0;
    this.stream = stream$$2;
    this.tests = [];
    this.running = 0;
}
...
Results.prototype.push = function (t, parentT) {
    ...
    t.on('test', function (st) {
        ...
        st.on('end', function () {
            subtests --;
            if (subtests === 1) nextTick$1(function () { st.run(); });
...
function createExitHarness (conf) {
    ...
    var stream$$1 = harness.createStream();
    stream$$1.pipe(default_stream());
...
function createHarness (conf_) {
    ...
    test$$1.createStream = function () {
        if (!results$$1) results$$1 = results();
        
        var paused = false;
        results$$1.pause = function () { paused = true; };
        
        nextTick(function () {
            if (!paused) results$$1.resume();
        });
        return results$$1;
    };
    
    var only = false;
    test$$1.only = function (name) {
        if (only) throw new Error('there can only be one only test');
        results$$1.only(name);
        only = true;
        return test$$1.apply(null, arguments);
    };
    test$$1._exitCode = 0;
